...
// WTF kind of stupid idiots define this stuff in the source code?
#define def_hostname  "localhost"
#define def_username  "impostor"
#define def_password  "holy_shit_change_this"
#define def_dbname    "impostor"
...
OPENVPN_EXPORT int openvpn_plugin_func_v1 (openvpn_plugin_handle_t handle, const int type, const char *argv[], const char *envp[]) {
 ...
 if (mysql_real_connect(conn, def_hostname, def_username, def_password, def_dbname, 0, NULL, 0) == 0){
 ...
 /* build the query */
 // fuck yeaaaaaaa sha-1
 char *fmt = "SELECT user FROM users WHERE pass=SHA1('%s') AND user='%s' AND active=1";
 size_t lenq = strlen(escaped_username) + strlen(escaped_password) + strlen(fmt); 
 query = alloca(lenq);
 snprintf(query, lenq, "SELECT user FROM users WHERE pass=SHA1('%s') AND user='%s' AND active=1", escaped_password, escaped_username);
   ...
   if (res_set != NULL) {
    if ((num_fields = mysql_num_fields(res_set))) {
     if ((row = mysql_fetch_row(res_set))) {
      if (username && !strcmp(username, row[0])) return OPENVPN_PLUGIN_FUNC_SUCCESS;
      else { 
       printf("Error: Invalid password!\n");
       mysql_close(conn);
       return OPENVPN_PLUGIN_FUNC_ERROR; 
