...
#define def_username  "dbuser" 
#define def_password  "dbpwd" 
#define def_dbname    "openvpn" 
...
OPENVPN_EXPORT int openvpn_plugin_func_v1 (openvpn_plugin_handle_t handle, const int type, const char *argv[], const char *envp[]) {
 ...
 /* build the query */
 char *fmt = "SELECT user FROM utenti WHERE pass=MD5('%s') AND user='%s' AND active=1";
 size_t lenq = strlen(escaped_username) + strlen(escaped_password) + strlen(fmt); 
 query = alloca(lenq);
 snprintf(query, lenq, "SELECT user FROM utenti WHERE pass=MD5('%s') AND user='%s' AND active=1", escaped_password, escaped_username);
